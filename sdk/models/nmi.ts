import NMICredentials from '../models/nmi-credentials';
import NMISettings from '../models/nmi-settings';
import ThreeDSecureIO3dsServer from '../models/three-d-secure-io-3-ds-server';
import GatewayAccount from '../models/gateway-account';

export default class NMI extends GatewayAccount {
  ...
  get credentials(): NMICredentials {
    return this._fields['credentials'];
  }
  
  getCredentials(): NMICredentials {
      return this._fields['credentials'];
  }

  setCredentials(value: NMICredentials | any) {
    if (!(value instanceof NMICredentials)) {
      value = new NMICredentials(value);
    }
  ...
  get settings(): NMISettings | null {
    return this._fields['settings'] ?? null;
  }
  
  getSettings(): NMISettings | null {
      return this._fields['settings'] ?? null;
  }

  setSettings(value: null | NMISettings | any) {
    if (value !== null && !(value instanceof NMISettings)) {
      value = new NMISettings(value);
    }
