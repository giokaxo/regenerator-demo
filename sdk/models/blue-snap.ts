import BlueSnapCredentials from '../models/blue-snap-credentials';
import BlueSnapSettings from '../models/blue-snap-settings';
import ThreeDSecureIO3dsServer from '../models/three-d-secure-io-3-ds-server';
import GatewayAccount from '../models/gateway-account';

export default class BlueSnap extends GatewayAccount {
  ...
  get credentials(): BlueSnapCredentials {
    return this._fields['credentials'];
  }
  
  getCredentials(): BlueSnapCredentials {
      return this._fields['credentials'];
  }

  setCredentials(value: BlueSnapCredentials | any) {
    if (!(value instanceof BlueSnapCredentials)) {
      value = new BlueSnapCredentials(value);
    }
  ...
  get settings(): BlueSnapSettings | null {
    return this._fields['settings'] ?? null;
  }
  
  getSettings(): BlueSnapSettings | null {
      return this._fields['settings'] ?? null;
  }

  setSettings(value: null | BlueSnapSettings | any) {
    if (value !== null && !(value instanceof BlueSnapSettings)) {
      value = new BlueSnapSettings(value);
    }
