import LaCoreCredentials from '../models/la-core-credentials';
import ThreeDSecureIO3dsServer from '../models/three-d-secure-io-3-ds-server';
import GatewayAccount from '../models/gateway-account';

export default class LaCore extends GatewayAccount {
  ...
  get credentials(): LaCoreCredentials {
    return this._fields['credentials'];
  }
  
  getCredentials(): LaCoreCredentials {
      return this._fields['credentials'];
  }

  setCredentials(value: LaCoreCredentials | any) {
    if (!(value instanceof LaCoreCredentials)) {
      value = new LaCoreCredentials(value);
    }
