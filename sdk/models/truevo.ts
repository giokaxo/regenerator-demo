import TruevoCredentials from '../models/truevo-credentials';
import ThreeDSecureIO3dsServer from '../models/three-d-secure-io-3-ds-server';
import GatewayAccount from '../models/gateway-account';

export default class Truevo extends GatewayAccount {
  ...
  get credentials(): TruevoCredentials {
    return this._fields['credentials'];
  }
  
  getCredentials(): TruevoCredentials {
      return this._fields['credentials'];
  }

  setCredentials(value: TruevoCredentials | any) {
    if (!(value instanceof TruevoCredentials)) {
      value = new TruevoCredentials(value);
    }
