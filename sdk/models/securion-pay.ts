import SecurionPayCredentials from '../models/securion-pay-credentials';
import ThreeDSecureIO3dsServer from '../models/three-d-secure-io-3-ds-server';
import GatewayAccount from '../models/gateway-account';

export default class SecurionPay extends GatewayAccount {
  ...
  get credentials(): SecurionPayCredentials {
    return this._fields['credentials'];
  }
  
  getCredentials(): SecurionPayCredentials {
      return this._fields['credentials'];
  }

  setCredentials(value: SecurionPayCredentials | any) {
    if (!(value instanceof SecurionPayCredentials)) {
      value = new SecurionPayCredentials(value);
    }
