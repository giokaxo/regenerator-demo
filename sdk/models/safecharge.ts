import SafechargeCredentials from '../models/safecharge-credentials';
import ThreeDSecureIO3dsServer from '../models/three-d-secure-io-3-ds-server';
import GatewayAccount from '../models/gateway-account';

export default class Safecharge extends GatewayAccount {
  ...
  get credentials(): SafechargeCredentials {
    return this._fields['credentials'];
  }
  
  getCredentials(): SafechargeCredentials {
      return this._fields['credentials'];
  }

  setCredentials(value: SafechargeCredentials | any) {
    if (!(value instanceof SafechargeCredentials)) {
      value = new SafechargeCredentials(value);
    }
