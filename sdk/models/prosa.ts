import ProsaCredentials from '../models/prosa-credentials';
import ThreeDSecureIO3dsServer from '../models/three-d-secure-io-3-ds-server';
import GatewayAccount from '../models/gateway-account';

export default class Prosa extends GatewayAccount {
  ...
  get credentials(): ProsaCredentials {
    return this._fields['credentials'];
  }
  
  getCredentials(): ProsaCredentials {
      return this._fields['credentials'];
  }

  setCredentials(value: ProsaCredentials | any) {
    if (!(value instanceof ProsaCredentials)) {
      value = new ProsaCredentials(value);
    }
